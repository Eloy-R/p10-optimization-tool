...
def _unique_orders_from_current_mix(arms_config: dict) -> List[Tuple[str, str, str, str]]:
    """
    Génère uniquement les permutations uniques du mix actuellement choisi
    sur les 4 bras dans l'interface.
    """
    base_order = [
        arms_config[1],
        arms_config[2],
...
def _score_row(
    ...
    Hiérarchie métier :
    1) production max
    2) latence la plus faible
    3) bonus léger sur taux four
    4) léger malus si pause longue et latence consigne élevée
    """
    score = (
        production * 1000
        - latence_moy * 10
...
def evaluate_optimization(
    ...
    - on garde exactement le mix choisi dans l'UI (bras 1..4)
    - on teste seulement les permutations uniques de ce mix
    - on teste les pauses 0 / 30 / 60
    - on teste plusieurs latences consignes
    """
...
def build_pause_latency_curve(df_scenarios: pd.DataFrame) -> pd.DataFrame:
    """
    Conservée pour compatibilité avec app.py.
    Si tu supprimes le graphe dans app.py, cette fonction peut rester telle quelle.
    """
    ...
    best_curve = work.groupby(
        ["Pause (min)", "Latence consigne (min)"],
        as_index=False
    ).first()
...
def evaluate_overtime_summary_from_best(
    ...
    """
    Synthèse overtime du meilleur scénario uniquement.
    Ne participe pas au classement principal.
    """
